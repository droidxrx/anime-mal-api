import { httpApi, MonthIsValid, getSeasonForNumberMonth, Obj } from "./utils.ts";
import { AnimeFullArray, AnimeInListArray, AnimeFull, AnimeInList } from "./structures.ts";
...
export default class anime {
     ...
     */
    // prettier-ignore
    search(q: string, offset: number = 0, limit: number = 100, fields: AnimeInListArray = AnimeInList): Promise<Obj> {
     ...
     */
    // prettier-ignore
    ranking(ranking_type: rankingtype = "all", offset: number = 0, limit: number = 100, fields: AnimeInListArray = AnimeInList): Promise<Obj> {
        fields = fields ? fields : AnimeInList
     ...
     */
    // prettier-ignore
    seasonal(year: number = new Date().getFullYear(), season: season = getSeasonForNumberMonth(new Date().getMonth()), offset: number = 0, limit: number = 100, sort: sort = "", fields: AnimeInListArray = AnimeInList): Promise<Obj> {
     ...
     * @param fields — array
     */
    suggestions(offset: number = 0, limit: number = 100, fields: AnimeInListArray = AnimeInList): Promise<Obj> {
        fields = fields ? fields : AnimeInList;
